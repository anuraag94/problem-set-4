...
```
4 is the optimum number of splits


###3. Natural Regression Spline
```{r}

test <- glm(egalit_scale ~ ns(as.factor(income06), df = 5),
               data = gss_train)

test$fitted
levels(test$fitted) <- factor(round(test$fitted))

test$fitted <- factor(test$fitted, levels = c(1:14, levels(test$fitted), 24:35))
